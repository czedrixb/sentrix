...
<template>
    <div>
        <header class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="font-heading text-2xl font-bold text-ink-700">Categories</h1>
        ...
        <div v-if="loading" class="space-y-3">
            <div v-for="n in 5" :key="n" class="h-14 animate-pulse rounded-lg bg-white" />
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="category in categories" :key="category.id">
                        <td class="px-4 py-3">
                            <p class="font-semibold text-ink-700">{{ category.name }}</p>
                            <p class="text-xs text-slate-500">/{{ category.slug }}</p>
                        </td>
                        <td class="px-4 py-3 text-slate-600">{{ parentName(category.parent_id) }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ category.position }}</td>
                    ...
                    <tr v-if="categories.length === 0">
                        <td colspan="4" class="px-4 py-8 text-center text-slate-400">No categories yet.</td>
                    </tr>
                </tbody>
            </table>
        </div>
