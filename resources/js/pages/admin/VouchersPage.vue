...
<template>
    <div>
        <header class="mb-6">
            <h1 class="font-heading text-2xl font-bold text-ink-700">Vouchers</h1>
        ...
        <p v-else-if="vouchers.length === 0" class="card p-12 text-center text-slate-500">No vouchers yet.</p>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="voucher in vouchers" :key="voucher.id">
                        <td class="px-4 py-3 font-semibold text-ink-700">{{ voucher.code }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ typeLabels[voucher.type] ?? voucher.type }}</td>
                        <td class="px-4 py-3 text-right">
                            {{ voucher.type.startsWith('percentage') ? `${voucher.value}%` : formatMoney(voucher.value) }}
                        </td>
                        <td class="px-4 py-3 text-right text-slate-500">{{ voucher.min_quantity ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            {{ voucher.times_used }}<span v-if="voucher.usage_limit" class="text-slate-400"> / {{ voucher.usage_limit }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <span
                                ...
                                :class="voucher.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'"
