...
<template>
    <div>
        <header class="mb-6">
            <h1 class="font-heading text-2xl font-bold text-ink-700">Staff</h1>
        ...
        <div v-if="loading" class="space-y-3">
            <div v-for="n in 5" :key="n" class="h-16 animate-pulse rounded-lg bg-white" />
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="user in users" :key="user.id">
                        <td class="px-4 py-3 font-semibold text-ink-700">{{ user.name }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ user.email }}</td>
                        <td class="px-4 py-3">
                            <span
                                v-for="role in user.roles"
                                :key="role"
                                class="mr-1 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600"
                            >{{ role }}</span>
                        </td>
