...
<template>
    <div class="mx-auto max-w-6xl px-4 py-10">
        <h1 class="font-display text-4xl text-ink-700">Your cart</h1>

        <div v-if="cart.isEmpty" class="card mt-8 p-12 text-center">
            <p class="font-heading text-lg font-semibold text-ink-700">Your cart is empty</p>
        ...
        <div v-else class="mt-8 grid gap-8 lg:grid-cols-[1fr_20rem]">
            <section class="space-y-4">
                <p class="text-sm text-slate-500">
                    Collecting from <strong class="text-ink-700">{{ cart.branch?.name }}</strong>
                </p>
                ...
                <article v-for="item in cart.items" :key="item.id" class="card flex gap-4 p-4">
                    ...
                    <div class="flex-1">
                        <RouterLink
                            ...
                            class="font-heading font-semibold text-ink-700 hover:text-brand-500"
                        >
                            {{ item.product.name }}
                        </RouterLink>
                    ...
                    <div class="text-right">
                        <p class="font-heading font-bold text-ink-700">{{ formatMoney(item.line_total) }}</p>
            ...
            <aside class="card h-fit p-5">
                <h2 class="font-heading text-lg font-bold text-ink-700">Summary</h2>
                ...
                <div class="mt-4 space-y-2 text-sm">
                    ...
                    <div class="flex justify-between border-t border-slate-200 pt-2 font-heading text-lg font-bold text-ink-700">
                        <span>Total</span>
